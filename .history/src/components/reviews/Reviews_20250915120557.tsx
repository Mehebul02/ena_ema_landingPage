...
export function ReviewsSection() {
  ...
  const intervalRef = useRef<number | null>(null)   // ✅ শুধু number

  // --- Auto play effect ---
  useEffect(() => {
    // আগের টাইমার মুছে ফেলা
    if (intervalRef.current) window.clearInterval(intervalRef.current)

    // হোভার না করলে অটো স্লাইড
    if (!isPaused) {
      intervalRef.current = window.setInterval(() => {
        setCurrentIndex(prev => (prev + 1) % testimonials.length)
      }, 3000) // 3 সেকেন্ডে একবার
    }
  ...
  const prevSlide = () => setCurrentIndex(p => (p - 1 + testimonials.length) % testimonials.length)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="hidden lg:grid lg:grid-cols-4 gap-6 mb-8">
          {testimonials.slice(0, 4).map(t => (
            <div key={t.id} className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              <div className="flex gap-1 pb-4">
                {[...Array(t.rating)].map((_,i)=><Star key={i} className="w-4 h-4 fill-green-500 text-green-500"/>)}
              </div>
        ...
        <div className="lg:hidden relative overflow-hidden">
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  i === currentIndex ? "bg-green-500 scale-110" : "bg-slate-600 hover:bg-slate-500"
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex justify-center gap-4 mt-4">
            <button onClick={prevSlide} className="p-2 rounded-full bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white">
              ◀
            </button>
            <button onClick={nextSlide} className="p-2 rounded-full bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white">
              ▶
            </button>
          </div>
        </div>
